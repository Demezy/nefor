...
Stage evidence retains workload, fixture/source, topology, forced-terminal, direct-prerequisite, and expected semantic/artifact fingerprints. Counter subtraction is `exclusive` only when the complete boundary matches, direct forcing is proved, and every counter delta is nonnegative. Lowering is always labeled **inclusive after proven analysis prefix** because `nefor.graph.lower` invokes analysis internally. Broad lowering also retains the full untimed lowered artifact observation.
