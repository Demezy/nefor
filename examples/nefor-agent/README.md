...
- `lead-workflow/` — re-export shims for the lead-workflow mechanism, which now lives in the shared lua tree at `lua/libs/lead-workflow/` (actor plan/approval state, kernel-run tracking, MAG source/application tools, and prompt loading). The complete lead and worker system prompts stay config-owned at `prompts/lead.md` and `prompts/worker.md`; they own orchestration and cross-tool selection policy, while `libs.mag-context` supplies canonical MAG reference material and provider-native tool schemas supply individual signatures. The loader adds only Nefor's canonical bounded-context instruction.
...
- `read-only-tools/` — a thin composition file selecting `discover_instruction_files` from `libs.read-only-tools`. The shared library also provides an opt-in ordinary `skill` loader and the `extra_tools` registration seam for configuration-owned capabilities. Repository discovery reports AGENTS.md/CLAUDE.md paths without loading their contents.
- `tool-validator/` — a thin composition file. The permission-classification mechanism (`shell.script`-via-`da` plus structural `process.exec`, edit/write policy, allowlist-derived read-only handling) ships at `lua/libs/tool-validator/`; this file parses the config-owned `mag/lib/nefor/toolsets.json` manifest and supplies its read-only inventory to the mechanism.
- `mag/lib/nefor/toolsets.json` — config-owned requested profiles for common delegated-agent capability surfaces. The canonical `nefor.toolsets` module in the registered `nefor-mag` package reads this manifest through MAG's typed JSON parser, while tool-validator uses the engine's JSON binding. The tool gate's owner-qualified runtime advertisement is the schema authority: MAG projects each requested profile through that snapshot before provider dispatch, and the original `:tools` names remain the invocation allowlist. Explicit custom lists are supported.
- `mag/lib/prompts/` — config-owned prompt fragments available as immutable file inputs. Canonical MAG modules and the MAG Book live once in the registered runtime package at `<runtime>/mag`; session workspaces contain only writable source. Reusable libraries are registered package roots rather than copied into sessions.
...
NEFOR_DEV_DIR=$PWD NEFOR_CONFIG_DIR=$PWD/examples/nefor-agent \
