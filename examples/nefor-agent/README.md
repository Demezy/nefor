...
- `mag/lib/prompts/` — config-owned prompt fragments available as immutable file inputs. Canonical MAG modules and the MAG Book live once in the registered runtime package at `<runtime>/mag`; session workspaces contain only writable source and optional local modules.
