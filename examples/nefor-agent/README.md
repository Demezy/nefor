...
- `lead-workflow/` — re-export shims for the lead-workflow mechanism, which now lives in the shared lua tree at `lua/libs/lead-workflow/` (actor plan/approval state, kernel-run tracking, mag-eval tool, and prompt loading). The complete lead and worker system prompts stay config-owned at `prompts/lead.md` and `prompts/worker.md`; the loader adds only Nefor's canonical bounded-context instruction.
