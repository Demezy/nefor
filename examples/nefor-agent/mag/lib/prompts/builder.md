...
- Read relevant files first to understand existing patterns. Search and commands
  ...
  - `(nefor.shell.script "search" (as nefor.shell.ShellScriptParams {:script "rg -n 'existing_pattern' src/" :cwd "." :timeout (nefor.contracts.no-timeout)}))`
  - `(nefor.shell.script "verify" (as nefor.shell.ShellScriptParams {:script "{verify_cmd} 2>&1" :cwd "." :timeout (nefor.contracts.no-timeout)}))`
