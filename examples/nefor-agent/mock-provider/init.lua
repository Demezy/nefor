...
local CANNED_MAG_PROGRAM = table.concat({
  ...
  '(let model (as nefor.actors.ResolvedModel {:provider "mock-plugin" :model "mock-model" :reasoning-effort "medium"}))',
  "",
  '(let start (nefor.graph.source "task"',
  '              (type-tag nefor.contracts.Task)',
  '              (as nefor.contracts.Task {:prompt "<initial task text>"})))',
  '(let sx (nefor.actors.agent exact-model',
  ...
  '(let sy (nefor.actors.agent exact-model',
  '           (as (nefor.actors.AgentConfig nefor.actors.ResolvedModel)',
  '             {:id "sy" :model model',
  ...
  '(let combine (nefor.actors.agent exact-model',
  '                (as (nefor.actors.AgentConfig nefor.actors.ResolvedModel)',
  '                  {:id "combine" :model model',
