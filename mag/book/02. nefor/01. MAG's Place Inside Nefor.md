...
changes allowed against a live run. MAG serializes the value supplied to
...
`empty-graph`, validates the complete result, lowers it, and returns a version-1
program envelope:
...
For an ordinary workflow, the transformation adds one already-composed node and
its output boundary. Before compilation, another pure function may add or remove
edges and return a new graph. Compilation does not retrieve, mutate, or cache a
stored graph.

A running graph changes through a `Delta`, which contains only actors, routes,
messages, kills, structural types, and logical paths for newly introduced
nodes:
...
The result is a version-1 delta envelope. The control plane may submit it with
...
version-1 expression vocabulary. Actors still have no authority to mutate the
graph.

The lead-agent `mag` tool uses the two boundaries directly. `action='apply'`
without `run_id` compiles a complete graph and executes its program envelope as
a fresh run. Supplying `run_id` compiles a delta file and submits its delta
...
Version 1 has no artifact cache, general runtime expression language,
