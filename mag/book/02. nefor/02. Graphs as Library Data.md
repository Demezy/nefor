...
whose complete input type accepts `Unit` gets exactly one bootstrap message during lowering.
An exact `Unit` boundary qualifies; a nominal ADT or product merely containing
...
and returns `Node<I, core.types.Result<AgentError, O>>`. The lower-level
