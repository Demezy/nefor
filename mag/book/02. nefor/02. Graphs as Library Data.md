...
the tool allowlist and normalized approval metadata as parameters. The current
`DaRules` map is forwarded as `da-policy`, but the tool gate does not enforce
those per-agent rules; its configured policy and validator own authorization. The dashed path is capability
