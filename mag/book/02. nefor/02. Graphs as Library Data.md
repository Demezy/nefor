...
A `Node I O` composes one or more actors, their internal routes, declarative operations,
...
whose input accepts `Unit` gets exactly one bootstrap message during lowering.
That includes a sum such as `Unit | Text`, but not a product merely containing
`Unit`. An incoming route or explicit input message keeps the node
...
functions. Version 2 has one operation,
...
extra actors hidden inside the node.
