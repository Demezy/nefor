...
completion; it is not a runtime-sized MAG container value. Consumers make the
interpretation explicit: `DynamicEach T` activates once per occurrence, while
`DynamicAll T` activates once with the ordered list after completion.

```mermaid
flowchart LR
  planner["planner: DynamicList Task"] -->|"DynamicEach Task"| expand["traverse node"]
  expand --> worker1["worker 1"]
  expand --> workerN["worker N"]
  worker1 -->|"DynamicList Result"| context["DynamicAll Result"]
  workerN --> context
...
closed worker template, and `nefor.dynamic.traverse-template` consumes
`DynamicEach` to instantiate it for each indexed occurrence.
`nefor.dynamic.context` consumes `DynamicAll`, waits for explicit completion,
and sends one ordered provider turn to the integrator.
