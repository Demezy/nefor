...
them once; it does not duplicate the actors or deliver an extra root
activation. Parent-owned connectors still remain distinct when their typed
endpoints differ, which is how one shared output fans out to independent
consumers. Reuse therefore means “the same runtime identity participates in
both structures,” not “run this computation twice” and not “cache a prior
result.” Use different actor IDs when two independent executions are intended.
...
must not split it to derive path segments or parentage. The run itself is the
display root, so its direct children appear immediately; there is no mandatory
one-node wrapper around the complete workflow.
...
and returns `Node<DynamicList<I>, DynamicList<O>>`. No public template node,
reference language, or separate dynamic-agent constructor is involved.
