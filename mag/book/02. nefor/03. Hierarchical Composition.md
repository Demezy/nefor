...
dynamic effect, `nefor.agents.dynamic-template-with-tools` defines the closed
worker template, and `nefor.dynamic.traverse-template` instantiates it for each
indexed occurrence. `nefor.dynamic.context` waits for explicit completion before
sending one ordered provider turn to the integrator.
