...
explicit per-item consumer operation. Use `nefor.dynamic.context` when the
consumer must activate once with the ordered collection
after completion. Fixed worker lists use `nefor.node.sequence`. Feedback that
requires an arbitrary runtime program or
post-compilation function call is outside the version-2 operation model; model
