...
facts; the reviewer performs a second judgment. A static pipeline can compose
these as ordinary nodes. A genuine feedback cycle adds resident rules only at
the points where runtime values choose a destination.
...
The workflow itself should be a short local node-producing function. Nefor's shared
library supplies graph, agent, process, and worktree mechanisms; it does not
choose this project's SDLC. The first complete progression—including a shared
command/policy value, a worktree, deterministic sequencing, typed choices, a
reusable development node, and later dynamic traversal—is the single program in
[Nefor MAG in Five Minutes](<00. Nefor MAG in Five Minutes.md>).

When the local SDLC needs the feedback arrows in the diagram, a resident
function may map `BuildFailed` or `NeedChanges` into a delta that sends the typed
value back to the builder. Another project may stop, retry through the ordinary
bounded retry node, or treat failure as its final business result. The library
therefore supplies mechanisms, not one prescribed error recipe.
