...
For runtime-sized work, use `nefor.dynamic.traverse-template`: it authors the closed
operation and exposes a `DynamicList` boundary. Fixed worker lists use
`nefor.node.sequence`. Feedback that requires an arbitrary runtime program or
