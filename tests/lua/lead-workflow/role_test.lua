...
assert_contains(lead, "Compilation validates the program; it is not approval for writes", "lead orders validation before approval")
assert_contains(lead, "Call `write-review` before applying", "lead orders approval before application")
assert_contains(lead, "quick terminal result is final", "lead recognizes synchronous completion")
assert_contains(lead, "call `await-run` once", "lead performs one attached wait after detachment")
assert_contains(lead, "Terminal findings from a mixed synchronous/asynchronous dispatch remain usable", "lead preserves partial terminal evidence")
assert_contains(lead, "withhold the final completion claim until each has delivered its canonical terminal result", "lead enforces the required-run completion barrier")
assert_excludes(lead, "Do not claim completion while a run is active", "lead does not let optional background work block partial synthesis")
assert_contains(lead, "Never poll `graph-status`", "lead forbids status polling")
assert_contains(lead, "Prefer structured process execution", "lead retains structural command policy")
assert_contains(lead, "Use a shell script only when an explicit POSIX shell program is required", "lead limits shell use")
assert_excludes(lead, "## MAG programs", "ambient context owns MAG tutorials")
assert_excludes(lead, "```lisp", "ambient context owns MAG syntax examples")
assert_excludes(lead, "A minimal agent program is", "ambient context owns canonical examples")
assert_excludes(lead, "Available MAG modules:", "ambient context owns the module inventory")
assert_excludes(lead, "Full MAG Book:", "ambient context owns the MAG Book reference")
assert_excludes(lead, "Writable source directory:", "ambient context owns workspace guidance")
