...
assert_eq(startup.parse({}).frontend, "tui", "default frontend remains TUI")
local cli = startup.parse({ "--frontend", "cli", "--resume", "s1", "--prompt", "hello", "--format", "json" })
assert_eq(cli.frontend, "cli")
assert_eq(cli.session_id, "s1")
assert_eq(cli.format, "json")
assert_error({ "--frontend", "cli" }, "CLI requires a nonempty --prompt")
assert_error({ "--frontend", "cli", "--resume", "s1" }, "CLI requires a nonempty --prompt")
assert_error({ "--frontend", "cli", "--prompt", " " }, "--prompt requires a prompt")
assert_error({ "--frontend", "other" }, "invalid frontend")
assert_error({ "--resume", "a", "--session", "b" }, "contradictory session selectors")
assert_error({ "--resume", "../a" }, "invalid session id")
assert_error({ "--format", "stream-json" }, "invalid format")
